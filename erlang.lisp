...
(defun parse-def (code &key (private nil))
  (flet ((emit (code &optional (dl 0))
	   (emit-erlang :code code :clear-env nil :level (+ dl 0))))
...
(defun emit-erlang (&key code (str nil) (clear-env nil) (level 0))
  ;(format t "emit ~a ~a~%" level code)
  (when clear-env
    (setf *env-functions* nil
	  *env-macros* nil))
  (flet ((emit (code &optional (dl 0))
	   (emit-erlang :code code :clear-env nil :level (+ dl level))))
